...
Для демонстрации удаленного управления включим SSH и настроим лини vty - разрешим доступ по ssh, установим пароль и привлегию линии повысим до высшей, чтоб миновать пароль enable.
...
Настроим проброс порта. При обращении на внешний адрес R14 будет происходить проброс на Loopback R19. 

```
ip nat inside source static tcp 10.10.50.19 22 10.10.100.2 22
```

Так как мы имеем два выхода во внешнюю сеть, через Киторн и через R15 внутри нашей AS, то надо проанонсировать сеть стыка с R15. Чтоб быть доступным с любой стороны.
...
Так же надо добавить outside и inside интерфейсы
...
После этого проверяем на R18 - подключаемся по SSH. После ввода команды видим предложение ввести пароль, после его введения оказываемся на R19. 

```
R18#ssh -l admin 10.10.100.2
Password:
R19#
```
